...
import { ReceivedPackingSlipsListService } from '../services/received-packing-slips-list.service';

@Component({
  selector: 'app-received-packing-slips-list',
  templateUrl: './received-packing-slips-list.component.html',
  styleUrls: ['./received-packing-slips-list.component.scss'],
})
@DestroySubscribers()
export class ReceivedPackingSlipsListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public receivedPackingSlipsListService: ReceivedPackingSlipsListService,
  ) {

  };

  ngOnInit() {
    this.packingSlips$ = this.receivedPackingSlipsListService.collection$;
  };

  addSubscribers() {
    this.subscribers.getAllCollectionSubscription = this.receivedPackingSlipsListService.getCollection()
    .subscribe();
  };
